                ...
                switch (event.error) {
                    ...
                    case 'service-not-allowed':
                        ...
                        if (new Date().getTime() - lastStartedAt < 200) {
                            invokeCallbacks(
                                callbacks.errorPermissionBlocked,
                                event
                            );
                        } else {
                            invokeCallbacks(
                                callbacks.errorPermissionDenied,
                                event
                            );
                        }
                        break;
            ...
            recognition.onend = function() {
                ...
                if (autoRestart) {
                    ...
                    var timeSinceLastStart =
                        new Date().getTime() - lastStartedAt;
                    autoRestartCount += 1;
            ...
            for (let phrase in commands) {
                if (commands.hasOwnProperty(phrase)) {
                    ...
                    if (typeof cb === 'function') {
                        ...
                        if (debugState) {
                            logMessage(
                                'Can not register command: %c' + phrase,
                                debugStyle
                            );
                        }
                        continue;
